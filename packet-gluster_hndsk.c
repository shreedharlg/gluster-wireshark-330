...
static int
gluster_hndsk_getspec_reply(tvbuff_t *tvb, int offset,
                                packet_info *pinfo _U_, proto_tree *tree)
{
        gchar* spec = NULL;
        offset = dissect_rpc_uint32(tvb, tree, hf_gluster_op_ret, offset);
...
static int
gluster_hndsk_getspec_call(tvbuff_t *tvb, int offset,
                                packet_info *pinfo _U_, proto_tree *tree)
{
        guint flags;
        gchar *key = NULL;
...
proto_reg_handoff_gluster_hndsk(void)
{
	...
	rpc_init_prog(proto_gluster_hndsk, GLUSTER_HNDSK_PROGRAM,
							ett_gluster_hndsk);
/*	rpc_init_proc_table(GLUSTER_HNDSK_PROGRAM, 2, gluster_hndsk_proc,
							hf_gluster_hndsk_proc);*/
